package com.app.vanshavali.listdetail.adapter
...
import com.app.vanshavali.databinding.ListDetailBinding
...
class CustomAdapter(
    ...
    var mList: List<MainEntity>,
    var isParentHasMultipleMarriage: String
) :
    ...
    fun updataList(mListUpdate: List<MainEntity>,   isMultipleMarriage: String) {
        mList = mListUpdate
        isParentHasMultipleMarriage  = isMultipleMarriage
    ...
    override fun onBindViewHolder(holder: MainViewHolder, position: Int) {
        ...
        runBlocking {
            launch {
                ...
                childCount = repository.getChildCount(ItemsViewModel.memberId)

            }
        }
        holder.bind(ItemsViewModel, childCount, isParentHasMultipleMarriage)
    ...
    class MainViewHolder(val binding: ListDetailBinding) : RecyclerView.ViewHolder(binding.root) {

        fun bind(mainEntity: MainEntity, childCount: Int, isParentHasMultipleMarriage: String) {
            binding.childName = mainEntity.nameEnglish
            binding.memberId = "User Id: ${mainEntity.memberId}"
            if(isParentHasMultipleMarriage.equals("2")){
                if(mainEntity.wifeId.equals("1")){
                    binding.isFirstOrSecondChild = "* Child with First marriage  "
                }else{
                    binding.isFirstOrSecondChild = "* Child with Second marriage  "
                }
            }else{
                binding.isFirstOrSecondChild = ""
            }
            binding.memberId = "User Id: ${mainEntity.memberId}"
            binding.noOfChild = "Children's: $childCount"
            ...
            fun create(parent: ViewGroup): MainViewHolder {
                ...
                return MainViewHolder(
                    DataBindingUtil.inflate(
                        ...
                        R.layout.listdetailsrow, parent,
                        false
                    )
                )
            }
